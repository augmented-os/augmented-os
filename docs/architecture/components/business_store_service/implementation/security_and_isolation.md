...
* **RLS Policy Management**: Create and maintain row-level security policies on any table that might contain multiple tenant data (mostly the metadata tables like `bss_metadata.tenant_schemas`). For tenant-specific tables (in separate schemas), ensure that even if accessed directly, a policy or at least a schema-based separation is always activ】.
...
ON bss_metadata.tenant_schemas
...
* **Overview** – high-level interactions and trust boundaries between BSS and other components (the overview touches on how Auth is used).
* **Configuration** – contains information on security-related configuration (like environment variables for Auth Service URL, public key rotation, encryption toggles).
* **Monitoring** – identifies key metrics and logs for security (e.g., metrics for failed auth attempts, logs for access denials).
* **API Reference** – each endpoint in the API Reference will detail the auth requirements (which roles, what token needed).
